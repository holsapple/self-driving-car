...
vector< vector<float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    // declare the initialBeliefs matrix (properly sized)
    vector<int>::size_type height = grid.size();
    vector<int>::size_type width = grid[0].size();
    vector< vector <float> > initialBeliefs (height, vector<float> (width));

    // define the initial (uniform) belief probability
    int area = height * width;
    float belief_per_cell = 1.0 / area;

    // construct the initialBelief matrix
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            initialBeliefs[row][col] = belief_per_cell;
        }
    }
...
vector< vector<float> > sense(char color, vector< vector<char> > grid, 
    ...
    // declare the nonNormBeliefs matrix (properly sized)
    vector<int>::size_type height = beliefs.size();
    vector<int>::size_type width = beliefs[0].size();
    vector< vector <float> > nonNormBeliefs (height, vector<float> (width));

    // construct the non-normalized new beliefs matrix
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            if (grid[row][col] == color) {
                nonNormBeliefs[row][col] = beliefs[row][col] * p_hit;
            }
            else {
                nonNormBeliefs[row][col] = beliefs[row][col] * p_miss;
            }
        }
    }

    return normalize(nonNormBeliefs);
}
...
/**
    ...
    @param blurring - A number representing how noisy robot 
        motion is. If blurring = 0.0 then motion is noiseless.
...
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, 
    ...
    // declare the pre-blurred newBeliefs matrix (properly sized)
    vector<int>::size_type height = beliefs.size();
    vector<int>::size_type width = beliefs[0].size();
    vector< vector <float> > newBeliefs (height, vector<float> (width));

    // declare a few variables needed in the code below
    float cell;
    int new_row;
    int new_col;

    // construct the pre-blurred newBeliefs matrix
    for (int row = 0; row < height; row++) {
        for (int col = 0; col < width; col++) {
            cell = beliefs[row][col];
            new_row = (row + dy) % height;
            new_col = (col + dx) % width;
            newBeliefs[new_row][new_col] = cell;
        }
    }

    return blur(newBeliefs, blurring);
}